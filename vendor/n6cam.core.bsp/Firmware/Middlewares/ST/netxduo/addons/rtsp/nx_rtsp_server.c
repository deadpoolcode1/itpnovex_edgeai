...
NX_RTSP_CLIENT *rtsp_client_ptr;
    ...
    for (i = 0; i < NX_RTSP_SERVER_MAX_CLIENTS; i++)
    {
         ...
         * FIX: SIANA: When ctrl+c is used to stop the client on Windows, the socket
         * is not closed properly and the state is set to NX_TCP_LISTEN_STATE (not handled
         * here initially). This causes the server to not be disconnected and next requests
         * are not processed (freezing the server).
         */
        if (
          (rtsp_client_ptr -> nx_rtsp_client_socket.nx_tcp_socket_state > NX_TCP_ESTABLISHED) ||
          (rtsp_client_ptr -> nx_rtsp_client_socket.nx_tcp_socket_state <= NX_TCP_LISTEN_STATE)
        )
        {
