...
/* Define various build options for the USBX port.  The application should either
 * make changes here by commenting or un-commenting the conditional compilation
 * defined OR supply the defines though the compiler's equivalent of the -D option. */
 ...
 * of bytes. The default is UX_ALIGN_16 (0x0f) to align allocated memory to 16
 * bytes. */
/*
...
/* Define control transfer timeout value in millisecond. The default is 10000 milliseconds. */
/* #define UX_CONTROL_TRANSFER_TIMEOUT            10000 */

/* Define non control transfer timeout value in millisecond. The default is 50000 milliseconds. */
/*
#define UX_NON_CONTROL_TRANSFER_TIMEOUT           50000
*/

/* Defined, this value is the maximum number of classes that can be loaded by
 * USBX. This value represents the class container and not the number of instances
 * of a class. For instance, if a particular implementation of USBX needs the hub
 * class, the printer class, and the storage class, then the UX_MAX_CLASSES value
...
/* Defined, this value represents the number of different host controllers available
 * in the system. For USB 1.1 support, this value will usually be 1. For USB 2.0
 * support, this value can be more than 1. This value represents the number of
 * concurrent host controllers running at the same time. If for instance there are
 * two instances of OHCI running, or one EHCI and one OHCI controller running,
 * the UX_MAX_HCD should be set to 2.  */
/*
#define UX_MAX_HCD                                1
*/

/* Defined, this value represents the maximum number of devices that can be attached
 * to the USB. Normally, the theoretical maximum number on a single USB is 127
 * devices. This value can be scaled down to conserve memory. Note that this value
 * represents the total number of devices regardless of the number of USB buses
 * in the system. */
/*
#define UX_MAX_DEVICES                            127
*/

/* Defined, this value represents the current number of SCSI logical units represented
 * in the device storage class driver. */
/*
#define UX_MAX_SLAVE_LUN                          1
*/

/* Defined, this value represents the maximum number of SCSI logical units represented
 * in the host storage class driver. */
/*
#define UX_MAX_HOST_LUN                           1
*/

/* Defined, this value represents the maximum number of bytes received on a control
 * endpoint in the device stack. The default is 256 bytes but can be reduced in memory
 * constrained environments. */
/*
#define UX_SLAVE_REQUEST_CONTROL_MAX_LENGTH       256
*/

/* Defined, this value represents the maximum number of bytes that can be received
 * or transmitted on any endpoint. This value cannot be less than the maximum packet
 * size of any endpoint. The default is 4096 bytes but can be reduced in memory
 * constrained environments. For cd-rom support in the storage class, this value
 * cannot be less than 2048.  */
#define UX_SLAVE_REQUEST_DATA_MAX_LENGTH          (2 * 1024)

/* Defined, this value includes code to handle storage Multi-Media Commands (MMC).
 * E.g., DVD-ROM. */
/*
#define UX_SLAVE_CLASS_STORAGE_INCLUDE_MMC
*/

/* Defined, this value represents the maximum number of bytes that a storage payload
 * can send/receive. The default is 8K bytes but can be reduced in memory constrained
 * environments.  */
/*
#define UX_HOST_CLASS_STORAGE_MEMORY_BUFFER_SIZE  (8 * 1024)
 */
...
/* Defined, this value represents the maximum number of Ed, regular TDs and Isochronous
 * TDs. These values depend on the type of host controller and can be reduced in
...
/* Defined, this value represents the maximum size of the HID decompressed buffer.
...
/*
#define UX_HOST_CLASS_HID_DECOMPRESSION_BUFFER    (4 * 1024)
*/

/* Defined, this value represents the maximum number of HID usages for a HID device.
 * Default is 2048 but for simple HID devices like keyboard and mouse it can be
 * reduced a lot. */
/*
#define UX_HOST_CLASS_HID_USAGES                  (2 * 1024)
*/
 ...
 * whenever there is a change in a key state i.e. when a key is pressed or released.
 * The report contains every key that is down). There are limitations to this method
 * such as not being able to determine when a key has been released.
 * Defined, this value causes ux_host_class_hid_keyboard_key_get to only report key
 * changes i.e. key presses and key releases. */
/*
...
/* Works when UX_HOST_CLASS_HID_KEYBOARD_EVENTS_KEY_CHANGES_MODE is defined.
 * Defined, this value causes ux_host_class_hid_keyboard_key_get to report modifier
 * key (Ctrl/Alt/Shift/GUI) changes. */
/*
 ...
 * Default is 8 but for memory constrained resource systems this can be reduced to 1. */
/*
#define UX_HOST_CLASS_STORAGE_MAX_MEDIA           2
*/

/* Defined, this value includes code to handle storage devices that use the CB or
 * CBI protocol (such as floppy disks). It is off by default because these protocols
 * are obsolete, being superseded by the Bulk Only Transport (BOT) protocol which
 * virtually all modern storage devices use. */
/*
#define UX_HOST_CLASS_STORAGE_INCLUDE_LEGACY_PROTOCOL_SUPPORT
*/

/* Defined, this value forces the memory allocation scheme to enforce alignment of
 * memory with the UX_SAFE_ALIGN field. */
/*
 ...
 * allocation until invoking the application's error callback and retrying. The
...
/* Defined, this value represents the the maximum number of HID events/reports that
 ...
 * The value is 3 bytes long (max 0xFFFFFFu). By default the bwPollTimeout is 1
 * (means 1ms).*/
/*
 ...
 * Only following USB-IF allowed device class code is allowed: 0x00, 0x02 (CDC
 * Control), 0x09 (Hub), 0x11 (Billboard), 0xDC (Diagnostic), 0xEF (MISC),
 * 0xFF (Vendor)
...
/* Define HID report transfer timeout value in millisecond. The default is 10000
 * milliseconds.  */
/*
...
/* Defined, this value will include the OTG polling thread. OTG can only be active
 * if both host/device are present. */
...
/* Defined, this macro will remove the FileX dependency of host storage. In this
 * mode, sector access is offered instead of directly FileX FX_MEDIA support. Use
 * following APIs for media obtain and access:
...
/*
#define UX_HOST_CLASS_STORAGE_MAX_TRANSFER_SIZE   (1 * 1024)
*/

/* Defined, this value represents the size of the log pool. */
#define UX_DEBUG_LOG_SIZE                         (8 * 1024)
 ...
 * If it's not defined, or TX is not included, a external function must be implement
 * in application:
